...
class trackingEvaluation(object):
    ...
    def __init__(
        ...
        self.t_sha = t_sha
        self.t_path = os.path.join(
            "./results/KITTI", t_sha, "data_%d" % (int(num_hypo) - 1)
        )
        print(self.t_path)
        ...
        if thres is None:
            if eval_2diou:
                self.min_overlap = (
                    0.5  # minimum bounding box overlap for 3rd party metrics
                )
            elif eval_3diou:
                self.min_overlap = (
                    0.25  # minimum bounding box overlap for 3rd party metrics
                )
            else:
                assert False
        else:
    ...
    def compute3rdPartyMetrics(self, threshold=-10000, recall_thres=1.0):
        ...
        for seq_idx in range(len(self.groundtruth)):
            ...
            for f in range(len(seq_gt)):  # go through each frame
                ...
                if tmptp + tmpfp + nignoredtp + nignoredtracker - nignoredpairs != len(
                    t
                ):
    ...
    def createSummary_details(self):
        ...
        summary += (
            "evaluation: best results with single threshold".center(80, "=") + "\n"
        )
        summary += (
...
class stat:
    ...
    def plot(self):
        ...
        self.plot_over_recall(
            ...
            "MOTA",
            os.path.join(
                save_dir, "MOTA_recall_curve_%s_%s.pdf" % (self.cls, self.suffix)
            ),
        )
        self.plot_over_recall(
            self.sMOTA_list,
            ...
            "sMOTA",
            os.path.join(
                save_dir, "sMOTA_recall_curve_%s_%s.pdf" % (self.cls, self.suffix)
            ),
        )
        self.plot_over_recall(
            self.motp_list,
            ...
            "MOTP",
            os.path.join(
                save_dir, "MOTP_recall_curve_%s_%s.pdf" % (self.cls, self.suffix)
            ),
        )
        self.plot_over_recall(
            self.f1_list,
            ...
            "F1",
            os.path.join(
                save_dir, "F1_recall_curve_%s_%s.pdf" % (self.cls, self.suffix)
            ),
        )
        self.plot_over_recall(
            self.fp_list,
            ...
            "False Positive",
            os.path.join(
                save_dir, "FP_recall_curve_%s_%s.pdf" % (self.cls, self.suffix)
            ),
        )
        self.plot_over_recall(
            self.fn_list,
            ...
            "False Negative",
            os.path.join(
                save_dir, "FN_recall_curve_%s_%s.pdf" % (self.cls, self.suffix)
            ),
        )
        self.plot_over_recall(
            self.precision_list,
            ...
            "Precision",
            os.path.join(
                save_dir, "precision_recall_curve_%s_%s.pdf" % (self.cls, self.suffix)
            ),
        )
...
# python3 scripts/KITTI/evaluate.py result_sha 1 3D 0.25
